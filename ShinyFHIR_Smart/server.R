...
con=dbConnect(drv,host="localhost",port="5432",user="postgres",dbname="fhirbase_pubtest",password="postgres")
...

{
 ...
 # length=dbGetQuery(con,"SELECT count(content) FROM observation WHERE content @> '{\"code\":{\"text\": \"oxygen_saturation\"}}'");
  len=as.numeric(length)
  print(len)
 out=dbGetQuery(con,"SELECT content  FROM observation")
#  out=dbGetQuery(con,"SELECT content FROM observation WHERE content @> '{\"code\":{\"text\": \"oxygen_saturation\"}}'");
       ...
       #   print(id)
        ...
        #  print(pid)
         # pid<-substr(pid, 9,length(pid))
          ...
          if(is.null(myobj$valueQuantity$value)){
            value="none";
          }
          print(value)
          system<-as.vector(myobj$valueQuantity$system)
          if(is.null(myobj$valueQuantity$system)){
            system="none";
          }
          print(system)
          unit<-as.vector(myobj$valueQuantity$unit)
          if(is.null(myobj$valueQuantity$unit)){
            unit="none";
          }
          print(unit)
        ...
        #  effectivePeriod_start<-as.vector(myobj$effectivePeriod$start)
          ...
          if(is.null(myobj$effectiveDateTime)){
            effectivePeriod_start="none";
          }
